...
import { computePhysicsSnapshot, type EnvironmentConditions } from "@bio-sim/simulation-kernel";
...
export class PhysicsEngine implements EngineLifecycle {
  ...
  advance(timestep: number, correlationId: string = randomUUID()): void {
    const { summary, metrics } = this.activeElement
      ? computePhysicsSnapshot({ element: this.activeElement, environment: this.environment })
      : {
          summary: "Physics engine awaiting active element selection.",
          metrics: {
...
export function createPhysicsEngine(bus: EventBus): PhysicsEngine {
  return new PhysicsEngine(bus);
}
